...
function route_areas(string $m, $id, array $b, array $q, $tid, $uid): never {
    $t = $tid(); Auth::need('dashboard', 'read');
    if ($m === 'GET') {
        if ($id) {
    ...
    switch ($m) {
        case 'GET':
            if ($id) {
                ...
                $outstanding = Db::val("SELECT COALESCE(SUM(i.total_amount-i.amount_paid),0) FROM invoices i JOIN sales_orders so ON so.id=i.order_id WHERE so.distributor_id=? AND i.tenant_id=? AND i.status NOT IN('Paid','Cancelled')", [$id, $t]) ?? 0;
            ...
            Http::ok(Db::all("SELECT d.*, a.name area_name,
                COALESCE((SELECT SUM(i.total_amount-i.amount_paid) FROM invoices i JOIN sales_orders so ON so.id=i.order_id WHERE so.distributor_id=d.id AND i.tenant_id=d.tenant_id AND i.status NOT IN('Paid','Cancelled')),0) outstanding
