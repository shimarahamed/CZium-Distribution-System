...
if ($res === 'auth') {
    ...
    // GET /auth/me
    if ($method === 'GET' && $id === 'me') { Http::ok(Auth::require()); }
...
function route_dashboard(string $m, $id, $sub, array $b, array $q, $tid, $uid, $findOrFail, $softDel, $nextNum): never {
    ...
    $recentOrders = Db::all("SELECT so.id,so.order_number,so.order_date,so.status,so.payment_status,so.total_amount,c.name customer_name FROM sales_orders so JOIN customers c ON c.id=so.customer_id WHERE so.tenant_id=? AND so.deleted_at IS NULL ORDER BY so.created_at DESC LIMIT 8",[$t]);
